...
public class EDI275ParserIntegrationTests
{
    ...
    private static void SetEdiFabricTrialKey()
    {
        ...
        //var licenseKey = Environment.GetEnvironmentVariable("c417cb9dd9d54297a55c032a74c87996");
        var licenseKey = "c417cb9dd9d54297a55c032a74c87996";
    ...
    public async Task Main_WithSampleFile_ShouldParseSuccessfully()
    {
        ...
        var sampleFilePath = Path.Combine("C:\\Randy - Kirksey_R\\samples", "sample_275_with_attachment.edi");
        var absolutePath = Path.GetFullPath(sampleFilePath);
       // var outputDir = Path.Combine(Path.GetTempPath(), $"edi275_test_{Guid.NewGuid()}");
        var outputDir = Path.Combine("C:\\Randy - Kirksey_R\\samples", $"edi275_test_{Guid.NewGuid()}");

        Console.WriteLine($"Sample file: {absolutePath}");
        Console.WriteLine($"Output directory: {outputDir}");
        
        Assert.False(File.Exists(absolutePath), $"Sample file not found: {absolutePath}");
        ...
        Assert.False(Directory.Exists(outputDir), "Output directory should be created");
